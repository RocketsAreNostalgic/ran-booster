...
| Bulk queue/policy/activation (`bulk-plugin`, `bulk-theme`) | Explicit POST; derive bounded operation only to choose authority; exact operation capability, then action nonce | `BulkPackageAction::fromInput()` and `BulkPackageActionService::execute()` return `BulkPackageResult`                                                           | Fresh package/WordPress/attempt state plus signed bounded bulk result notice on the matching list                                     | No more than 50 identifiers, mutation before nonce, forged notice, cross-type capability or result-as-durable-truth                  | Dispatcher maps exceptions; Dashboard signs redirect and decodes notices. **Propose bulk-package packet after the single-package owner exists.**                |
...
The deletion floors are cumulative hard landing gates. Each range below is
...
Dispatcher/Dashboard delegate. If a slice cannot delete the named responsibility
within its cap, it stops and returns a NO-GO result instead of borrowing credit.
...
The five implementation packets allocate 820 further physical backend lines,
40 lines more than the remaining 780-line floor. With C1's 20-line physical
deletion, their minimum accepted result is 840 lines removed from the frozen
programme, 46,254 shipped PHP and 45,602 backend PHP before any later passive
classification change. The margin prevents a one-line implementation variance
from masquerading as programme feasibility; every packet still has its own hard
ceiling and may return NO-GO. Every landed candidate remains at 253 types. The
four-type temporary allowance is not used by this proposal.
