...
| Booster Core                  | Owns package source and revision, deployment policy, facade authorization, prospective adoption, managed-package enumeration, native-update authority snapshots and locking, stale-offer suppression, branch admission/history, branch preflight, and final management postconditions. |
| Shared GitHub release updater | Owns release discovery, channel selection, caching, stable repository-ID checks, exact release/ZIP/digest identity, package-header validation, archive custody, optional rejection-only assurance, and its provider-owned native WordPress target runtime. |
| Repository provider           | Resolves branch and release operations, owns provider credentials, constructs and registers its optional native targets, normalizes their bounded passive status and refresh, and prepares authenticated archives and expected-head verification when required. |
