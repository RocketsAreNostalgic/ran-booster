...
The previous sibling Git checkout of the neutral updater is intentionally unnecessary: it was not a source of staged bytes. Exact repository/ref identity is proved by the lock-plus-policy verifier, while the clean Composer install supplies the bytes that are staged and independently compared. Installed WordPress readback still proves the neutral updater's runtime metadata and V1 marker, deriving its package root and version from the same validated policy/lock projection rather than duplicating those identities in CI.
