...
`scripts/verify-runtime-dependencies.php` validates the policy schema and the committed lock together. It rejects unexpected or duplicate production packages, dot-segment package names, malformed SemVer versions/references, repository/source mismatches, invalid or duplicate top-level surfaces, and an invalid neutral-updater role. Its normal output reports the resolved package/version/reference records; `--packaging` adds the validated repository/root/typed-surface projection for build, verification, and CI readback. The `--verify-install` mode applies that same policy to a clean Composer install and rejects wrong-kind surfaces, symbolic links, unsupported filesystem entries, and empty directories anywhere inside a declared directory surface.

`scripts/build-release.sh` and `scripts/verify-release.sh` consume that validated projection generically and invoke the shared installed-surface validation before staging or reconstructing archive expectations. They do not maintain package-specific roots, surfaces, versions, repositories, or commit constants. Both scripts also retain their local fail-closed surface checks. The builder stages only policy-approved surfaces from a clean `composer install --no-dev`; the verifier independently reconstructs the expected archive file set and compares every packaged runtime file byte-for-byte with the clean install.
