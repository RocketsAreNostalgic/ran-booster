...
function add_filter( string $hook, callable $callback, int $priority = 10, int $acceptedArgs = 1 ): bool {
	$GLOBALS['ran_booster_github_webhook_management_filters'][ $hook ][] = compact( 'callback', 'priority', 'acceptedArgs' );
...
function wp_enqueue_style( string $handle, string $source, array $dependencies = array(), string|bool|null $version = false ): void {
	$GLOBALS['ran_booster_github_webhook_management_styles'][] = compact( 'handle', 'source', 'dependencies', 'version' );
}
