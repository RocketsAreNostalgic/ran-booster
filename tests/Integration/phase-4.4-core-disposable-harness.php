...
function phase44_github_response( array $fixture, array $args, string $url ): ?array {
	...
	$headers = $args['headers'] ?? null; if ( ! in_array( $url, $known, true ) || ! is_array( $headers ) || 'Bearer phase44-token' !== ( $headers['Authorization'] ?? null ) || 'ran-wp-release-updater/0.1.0-beta.1' !== ( $headers['User-Agent'] ?? null ) || ( $asset === $url ? 'application/octet-stream' : 'application/vnd.github+json' ) !== ( $headers['Accept'] ?? null ) ) return null;
