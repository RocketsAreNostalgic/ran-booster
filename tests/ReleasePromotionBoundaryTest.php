...
final class ReleasePromotionBoundaryTest extends TestCase {
	public function testReleasePleaseConfigurationUsesProfileBDraftLifecycle(): void {
		$config = json_decode( $this->readText( 'release-please-config.json' ), true, 512, JSON_THROW_ON_ERROR );
	...
	public function testProfileBCallerOwnsTheOnlyGenericReleaseMutationBoundary(): void {
		...
		self::assertStringContainsString( 'uses: RocketsAreNostalgic/.github/.github/workflows/release-profile-b.yml@e2fb19244a301a62f8fae2a80536898adf21fe22', $workflow );
	...
	public function testQualityPromotionManifestBindsExactCoreAssets(): void {
		...
		foreach ( array( 'ran-profile-b-promotion', 'quality_commit:$quality_commit', 'source_commit:$source_commit', 'ran-booster-${version}.zip', '${archive}.sha256', 'archive_sha256', 'checksum_sha256' ) as $marker ) {
	...
	public function testObsoleteGenericReleaseStateHelpersAreRemoved(): void {
		...
		) as $path ) self::assertFileDoesNotExist( $this->root() . '/' . $path, $path );
	}
	...
	public function testReleaseDocumentationMapsRetainedAndDeletedGuarantees(): void {
		...
		self::assertStringContainsString( 'Mutable `--clobber` recovery', $release );
	}
