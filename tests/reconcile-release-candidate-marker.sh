...
linear_fixture=$(jq \
	...
	--arg bot 'github-actions[bot]' \
	--arg bot_email '41898282+github-actions[bot]@users.noreply.github.com' \
	'.identity.data.repository.object.parents = {totalCount: 1, nodes: [{oid: $base}]}
	| .identity.data.repository.object.author = {email: $bot_email, user: {login: $bot}}' \
	<<< "$fixture")
...
invalid_linear_bot_identity=$(jq '.identity.data.repository.object.signature.isValid = false' <<< "$linear_fixture")
...
invalid_identity=$(jq '.identity.data.repository.object.signature.isValid = false' <<< "$fixture")
...
invalid_generated_identity=$(jq '.identity.data.repository.object.parents.nodes[0].signature.isValid = false' <<< "$fixture")
...
wrong_generated_author=$(jq '.identity.data.repository.object.parents.nodes[0].author.user.login = "attacker"' <<< "$fixture")
...
missing_base=$(jq '.identity.data.repository.object.parents.nodes[1].oid = "5555555555555555555555555555555555555555"' <<< "$fixture")
...
truncated_parents=$(jq '.identity.data.repository.object.parents.totalCount = 3' <<< "$fixture")
