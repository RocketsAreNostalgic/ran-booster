	...
	<div class="ran-booster-page-shell__body">
		...
		<details id="ran-booster-credential-storage" class="ran-booster-documentation__section ran-booster-panel">
			...
			<div class="ran-booster-documentation__content">
				...
				<p><?php esc_html_e( 'Booster stores repository credentials and webhook secrets as authenticated ciphertext in a private JSON file outside the WordPress and plugin directories. Conventional single-site POSIX installations can use the protected Overview’s automatic location. Containers and uncommon layouts can define', 'ran-booster' ); ?> <code>RAN_BOOSTER_ENCRYPTED_SECRETS_FILE</code> <?php esc_html_e( 'as an absolute secrets.json path outside the public web root on durable local storage whose immediate parent is owned by PHP, readable and writable by PHP, and mode 0700. Booster never displays a saved secret back to you once it is stored.', 'ran-booster' ); ?></p>
				...
				<p><?php esc_html_e( 'A plugin ZIP export or a database-only site migration cannot recreate the encrypted store. Transporter, described above, is the supported way to move selected repository credentials: it protects them inside the Transporter Blueprint ZIP, confirms repository identity and access on the target, re-encrypts them with the target site’s key, then associates each validated credential with the same packages when you apply it — including public repositories that deliberately use authenticated access. Constants, webhook secrets and credentials already saved on the target are never exported. When Transporter is not the right fit, add credentials directly through the provider settings screens on the target site, or use the supported deployment constants instead.', 'ran-booster' ); ?></p>
		...
		<details id="ran-booster-push-to-deploy" class="ran-booster-documentation__section ran-booster-panel">
			...
			<div class="ran-booster-documentation__content">
				...
				<p><?php esc_html_e( 'When Assisted Hooks has an identified hook record, its verified Remove action can remove that remote hook and release only a repository secret that the add-on created specifically for it. Without the add-on, open the repository webhook settings from the provider screen, remove the hook at the provider, then use Manage secrets to remove an unused local secret. If ownership or remaining use is uncertain, leave the setup in place.', 'ran-booster' ); ?></p>
